...
#cols <- c( minmatarName="red","Basilisk"="blue","Guardian"="orange","Oneiros"="green")
#print (cols)

cols <- c( "Prowler"="red","Crane"="steelblue2","Prorator"="orange","Viator"="aquamarine4")
# print (cols)

graphtitle = paste("Blockade Runner ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat12735, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat12735, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat12729, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat12729, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat12733, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat12733, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat12743, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat12743, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="BlockadeRunner",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","BlockadeRunner_",runyear,".png",sep="")
