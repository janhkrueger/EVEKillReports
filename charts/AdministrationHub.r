...
graphtitle = paste("Administration Hub ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat35842, aes(x = datum, y = anzahl, colour="Medium") ) + geom_point(data=dat35842, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat35843, aes(x = datum, y = anzahl, colour="Large") + geom_point(data=dat35843, aes(x = datum, y = anzahl), size=1 ) + 
#  geom_line(data=dat35844, aes(x = datum, y = anzahl, colour="X-Large") ) + geom_point(data=dat35844, aes(x = datum, y = anzahl), size=1 ) + 
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Administration Hub",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","AdministrationHub_",runyear,".png",sep="")
