...
cols <- c( "Scalpel"="red","Kirin"="steelblue2","Deacon"="orange","Thalia"="aquamarine4")

graphtitle = paste("Logistic Frigates Tech 2 ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat37460, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat37460, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat37458, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat37458, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37457, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat37457, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37459, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat37459, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="LogiFrigsT2",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","LogisticFrigatesTech2_",runyear,".png",sep="")
