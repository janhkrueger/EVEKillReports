...
cols <- c( "Lif"="red","Minokawa"="steelblue2","Apostole"="orange","Ninazu"="aquamarine4")
# print (cols)

graphtitle = paste("Force Auxiliary ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat37606, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat37606, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat37605, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat37605, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37604, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat37604, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37607, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat37607, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="FAUX",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","ForceAuxiliary_",runyear,".png",sep="")
