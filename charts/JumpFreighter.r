...
cols <- c( "Nomad"="red","Rhea"="steelblue2","Ark"="orange","Anshar"="aquamarine4")

graphtitle = paste("Jump Freighter ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat28846, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat28846, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat28844, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat28844, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat28850, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat28850, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat28848, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat28848, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="JF",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","JumpFreighter_",runyear,".png",sep="")
