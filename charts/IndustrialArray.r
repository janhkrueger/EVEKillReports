...
graphtitle = paste("Industrial Array ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat35825, aes(x = datum, y = anzahl, colour="Medium") ) + geom_point(data=dat35825, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat35826, aes(x = datum, y = anzahl, colour="Large") + geom_point(data=dat35826, aes(x = datum, y = anzahl), size=1 ) + 
#  geom_line(data=dat35827, aes(x = datum, y = anzahl, colour="X-Large") ) + geom_point(data=dat35827, aes(x = datum, y = anzahl), size=1 ) + 
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Industrial Array",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","IndustrialArray_",runyear,".png",sep="")
