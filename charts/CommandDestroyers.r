...
cols <- c( "Bifrost"="red","Stork"="steelblue2","Pontifex"="orange","Magus"="aquamarine4")
# print (cols)

graphtitle = paste("Command Destroyers ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat37480, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat37480, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat37482, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat37482, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37481, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat37481, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat37483, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat37483, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="CMDD",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","CommandDestroyers_",runyear,".png",sep="")
