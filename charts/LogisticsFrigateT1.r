...
cols <- c( "Burst"="red","Bantam"="steelblue2","Inquisitor"="orange","Navitas"="aquamarine4")

graphtitle = paste("Logistic Frigates Tech 1 ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat599, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat599, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat582, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat582, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat590, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat590, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat592, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat592, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="LogiFrigsT1",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","LogisticFrigatesTech1_",runyear,".png",sep="")
