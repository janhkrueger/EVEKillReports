...
cols <- c("Hound"="red","Manticore"="steelblue2","Purifier"="orange","Nemesis"="aquamarine4")

graphtitle = paste("Stealth Bomber ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat12034, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat12034, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat12032, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat12032, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat12038, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat12038, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat11377, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat11377, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Stealth Bomber",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","StealthBomber_",runyear,".png",sep="")
