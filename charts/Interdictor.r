...
cols <- c("Sabre"="red","Flycatcher"="steelblue2","Heretic"="orange","Eris"="aquamarine4")

graphtitle = paste("Interdictor ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat22456, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat22456, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat22464, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat22464, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat22452, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat22452, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat22460, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat22460, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Interdictor",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","Interdictor_",runyear,".png",sep="")
