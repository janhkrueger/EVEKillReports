...
graphtitle = paste("Drilling Platform ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat35835, aes(x = datum, y = anzahl, colour="Medium") ) + geom_point(data=dat35835, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat35836, aes(x = datum, y = anzahl, colour="Large") + geom_point(data=dat35836, aes(x = datum, y = anzahl), size=1 ) + 
#  geom_line(data=dat35837, aes(x = datum, y = anzahl, colour="X-Large") ) + geom_point(data=dat35837, aes(x = datum, y = anzahl), size=1 ) + 
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Drilling Platform",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","DrillingPlatform_",runyear,".png",sep="")
