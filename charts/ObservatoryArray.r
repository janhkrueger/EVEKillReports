...
graphtitle = paste("Observatory Array ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat35838, aes(x = datum, y = anzahl, colour="Medium") ) + geom_point(data=dat35838, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat35839, aes(x = datum, y = anzahl, colour="Large") + geom_point(data=dat35839, aes(x = datum, y = anzahl), size=1 ) + 
#  geom_line(data=dat37533, aes(x = datum, y = anzahl, colour="X-Large") ) + geom_point(data=dat37533, aes(x = datum, y = anzahl), size=1 ) + 
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Observatory Array",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","ObservatoryArray_",runyear,".png",sep="")
