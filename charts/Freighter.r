...
# Abfrage Ore
# Den neuen Namen zusammensetzen und Variable zuweisen
tempname = paste("dat", oreID, sep = "")
assign(tempname, getKills(runyear, oreID)  )


cols <- c( "Fenrir"="red","Charon"="steelblue2","Providence"="orange","Obelisk"="aquamarine4", "Bowhead"="grey")

graphtitle = paste("Freighter ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat20189, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat20189, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat20185, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat20185, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat20183, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat20183, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat20187, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat20187, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat34328, aes(x = datum, y = anzahl, colour=oreName) ) + geom_point(data=dat34328, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Freighter",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","Freighter_",runyear,".png",sep="")
