...
graphtitle = paste("Laboratory ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat35828, aes(x = datum, y = anzahl, colour="Medium") ) + geom_point(data=dat35828, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat35829, aes(x = datum, y = anzahl, colour="Large") + geom_point(data=dat35829, aes(x = datum, y = anzahl), size=1 ) + 
#  geom_line(data=dat35830, aes(x = datum, y = anzahl, colour="X-Large") ) + geom_point(data=dat35830, aes(x = datum, y = anzahl), size=1 ) + 
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Laboratory",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","Laboratory_",runyear,".png",sep="")
