...
cols <- c( "Scythe"="red","Osprey"="steelblue2","Augoror"="orange","Exequror"="aquamarine4")

graphtitle = paste("Logistic Cruisers Tech 1 ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat631, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat631, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat620, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat620, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat625, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat625, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat634, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat634, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="LogiCruisersT1",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","LogisticCruisersTech1_",runyear,".png",sep="")
