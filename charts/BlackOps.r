...
cols <- c("Panther"="red","Widow"="steelblue2","Redeemer"="orange","Sin"="aquamarine4")

graphtitle = paste("Black Ops ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat22440, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat22440, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat22436, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat22436, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat22428, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat22428, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat22430, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat22430, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Black Ops",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","BlackOps_",runyear,".png",sep="")
