...
# Abfrage Etana
# Den neuen Namen zusammensetzen und Variable zuweisen
#tempname = paste("dat", etanaID, sep = "")
#assign(tempname, getKills(runyear, etanaID)  )

cols <- c( "Scimitar"="red","Basilisk"="steelblue2","Guardian"="orange","Oneiros"="aquamarine4", "Etana"="grey")

graphtitle = paste("Logistic Cruisers Tech 2 ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat11978, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat11978, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat11985, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat11985, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat11987, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat11987, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat11989, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat11989, aes(x = datum, y = anzahl), size=1 ) +
#  geom_line(data=dat32790, aes(x = datum, y = anzahl, colour=etanaName) ) + geom_point(data=dat32790, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="LogiCruisersT2",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","LogisticCruisersTech2_",runyear,".png",sep="")
