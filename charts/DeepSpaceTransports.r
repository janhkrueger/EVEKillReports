...
cols <- c( "Mastodon"="red","Bustard"="steelblue2","Impel"="orange","Occator"="aquamarine4")
# print (cols)

graphtitle = paste("Deep Space Transports ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat12747, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat12747, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat12731, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat12731, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat12753, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat12753, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat12745, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat12745, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="DST",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","DeepSpaceTransports_",runyear,".png",sep="")
