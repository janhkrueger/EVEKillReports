...
cols <- c("Hyena"="red","Kitsune"="steelblue2","Sentinel"="orange","Keres"="aquamarine4")

graphtitle = paste("Electronic Attack Ship ",runyear,sep="")

# Plotting all four into one
p <- ggplot() + 
  ggtitle(graphtitle) + 
  geom_line(data=dat11387, aes(x = datum, y = anzahl, colour=minmatarName) ) + geom_point(data=dat11387, aes(x = datum, y = anzahl), size=1 ) +
  geom_line(data=dat11194, aes(x = datum, y = anzahl, colour=caldariName) ) + geom_point(data=dat11194, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat11190, aes(x = datum, y = anzahl, colour=amarrName) ) + geom_point(data=dat11190, aes(x = datum, y = anzahl), size=1 ) + 
  geom_line(data=dat11174, aes(x = datum, y = anzahl, colour=gallenteName) ) + geom_point(data=dat11174, aes(x = datum, y = anzahl), size=1 ) +
  # Setting the labels
  xlab("Week") +
  ylab("Losses") +
  scale_colour_manual(name="Electronic Attack Ship",values=cols, guide = guide_legend(fill = NULL,colour = NULL) ) +
...
dateiname = paste("/var/games/KillReporter/EVEData/charts/","ElectronicAttackShip_",runyear,".png",sep="")
