SELECT ci.solarSystemName, mr.regionName, round((sum(ci.copyIndex)*100),2) as sumcopyindex, round(avg(mss.security),2) as avgsecurity, IF( avg(mss.security) >=0.5, 'HighSec', IF( avg(mss.security) <=0.0, 'NullSec', 'LowSec')) as SystemSecurity, ci.surveyDate
